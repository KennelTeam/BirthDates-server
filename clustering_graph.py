...
def generalize_item(keywords: dict):
    keywords_sorted = sorted([(keywords[word], word) for word in keywords if len(
        wn.synsets(word)) > 0], reverse=True)
    ...
    for k, word in keywords_sorted:
        ...
        for idx, hyper in enumerate(hypers):
            ...
            if hyper in new_synsets:
                ...
                new_synsets[hyper] = k / (idx + 1)**0.5
    # order new synsets
    synsets_sorted = sorted([(new_synsets[syn], syn)
                            for syn in new_synsets], reverse=True)
    # choose 15 the most
...
def make_clustering_predata(steps: int, initial_data):
    ...
    for i in range(steps):
        ...
        new_data, new_labels = clustering_step(
            cluster_words[-1], CLUSTER_DENSITY ** (steps - i), generalize_item_v3)
